...
class StreamProgress(
  ...
  def removed(key: SparkDataStream): Map[SparkDataStream, OffsetV2] = baseMap - key

  def ++(updates: IterableOnce[(SparkDataStream, OffsetV2)]): StreamProgress = {
    new StreamProgress(baseMap ++ updates)
  }

  override def +[V1 >: OffsetV2](kv: (SparkDataStream, V1)): Map[SparkDataStream, V1] = {
    new StreamProgress(baseMap + kv)
  }

  override def -(key: SparkDataStream): StreamProgress = new StreamProgress(baseMap - key)
}
